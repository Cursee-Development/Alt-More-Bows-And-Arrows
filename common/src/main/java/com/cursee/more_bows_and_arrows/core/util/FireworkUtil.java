...
//    }
    ...
    private static boolean noFireworkExplosionSet(ItemStack stack) {
        // if (stack.getTag() == null || !stack.hasTag()) return false;
        if (stack.get(DataComponents.FIREWORKS) == null || stack.getComponents().isEmpty()) return false;
        // return !stack.getTag().contains("Fireworks") || !stack.getTag().getCompound("Fireworks").contains("Explosions");
        return !stack.getComponents().has(DataComponents.FIREWORKS) || !stack.get(DataComponents.FIREWORKS).explosions().isEmpty();
    }
...
//            newRandomFireworkItemStack.setTag(compoundTagFromFireworkItemStack);
            ...
            List<FireworkExplosion> explosions = new ArrayList<FireworkExplosion>();

            FireworkExplosion hardCoded = new FireworkExplosion(FireworkExplosion.Shape.BURST, IntList.of(0),IntList.of(0), true, true);
            ...
            ItemStack fireworkRocketItem = Items.FIREWORK_ROCKET.getDefaultInstance();
            fireworkRocketItem.set(DataComponents.FIREWORKS, fireworks);
