...
public class ArrowHitsEntityInteractions {
    ...
    public static void interact(TieredArrowEntity tieredArrowEntity, @Nullable Entity nullableOwner, EntityHitResult entityHitResult) {
        ...
        switch (tieredArrowEntity.getVariant()) {
            ...
            case BLAZE_ROD -> hitEntity.setRemainingFireTicks(2 * 20);
            ...
            case FIRE_CHARGE -> hitEntity.setRemainingFireTicks(2 * 20);
            case FLINT -> {
                if (player.getMainHandItem().getItem() instanceof TieredBowItem bow && bow.tier == BowTier.IRON) hitEntity.setRemainingFireTicks(2 * 20);
            }
